...
export const analyzeTailEnd12WeekLookback = (grid, cols = 7) => {
  ...
  const startTargetRow = Math.max(2, lastFilledR - 3);
  const endTargetRow = Math.min(rows - 1, lastFilledR + 1);

  for (let r = startTargetRow; r <= endTargetRow; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      // 1. Vertical Setup (Same Column): (r-2, c) -> (r-1, c)
      if (r >= 2) {
        setupTypes.push({
          direction: 'Vertical (Same Col)',
          cell1: { r: r - 2, c },
          cell2: { r: r - 1, c }
        });
      }

      // 2. Horizontal Setup (Same Row): (r, c-2) -> (r, c-1)
      if (c >= 2) {
        setupTypes.push({
          direction: 'Horizontal (Same Row)',
          cell1: { r, c: c - 2 },
          cell2: { r, c: c - 1 }
        });
      }

      // 3. Diagonal Setup: (r-1, c-1) -> (r-1, c)
      if (r >= 1 && c >= 1) {
        setupTypes.push({
          direction: 'Diagonal (Cross)',
          cell1: { r: r - 1, c: c - 1 },
          cell2: { r: r - 1, c }
        });
      }
      ...
      setupTypes.forEach(setup => {
        ['open', 'close'].forEach(digitType => {
          const d1 = getDigit(setup.cell1.r, setup.cell1.c, digitType);
          const d2 = getDigit(setup.cell2.r, setup.cell2.c, digitType);

          if (d1 !== null && d2 !== null) {
            const tailPatternStr = `${d1} ➔ ${d2}`;
            const lookbackMinR = Math.max(0, r - 12);
            ...
            for (let histR = r - 2; histR >= lookbackMinR; histR--) {
              // Vertical check in history
              ...
              if (hd1 === d1 && hd2 === d2) {
                ...
                if (followR < rows) {
                  ...
                  if (followVal && followVal !== '**' && /^\d{2}$/.test(followVal)) {
                    ...
                    matchesIn12Week.push({ histR, followR, followVal, fOpen, fClose });
                  }
                }
              }

              // Horizontal check in history
              for (let hc = 0; hc < cols - 2; hc++) {
                const hhd1 = getDigit(histR, hc, digitType);
                const hhd2 = getDigit(histR, hc + 1, digitType);
                if (hhd1 === d1 && hhd2 === d2) {
                  const followVal = grid[histR]?.[hc + 2]?.val;
                  if (followVal && followVal !== '**' && /^\d{2}$/.test(followVal)) {
                    totalMatches++;
                    const fOpen = followVal[0];
                    const fClose = followVal[1];
            ...
            if (totalMatches > 0) {
              ...
              const item = {
                id: `tail_pred_${r}_${c}_${setup.direction}_${digitType}`,
                r,
                c,
                rowNum: r + 1,
              ...
              colPredictions.push(item);

              const key = `${r}_${c}`;
              if (!emptyCellMap[key]) emptyCellMap[key] = [];
              emptyCellMap[key].push(item);
            }
          }
        });
      });
