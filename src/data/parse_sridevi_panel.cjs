  ...
  res.on('end', () => {
    ...
    while ((match = trRegex.exec(html)) !== null) {
      ...
      while ((tdMatch = tdRegex.exec(trContent)) !== null) {
        // preserve inner line breaks or tags as spaces
        let text = tdMatch[1]
          .replace(/<br\s*\/?>/gi, ' ')
          .replace(/<[^>]+>/g, ' ')
          .replace(/\s+/g, ' ')
    ...
    parsedRows.forEach(row => {
      ...
      if (rowText.includes('MON') || rowText.includes('DATE') || rowText.includes('SRIDEVI MATKA') || rowText.includes('RECORD')) return;
      ...
      for (let cIdx = 0; cIdx < 7; cIdx++) {
        ...
        } else if (parts.length === 1 && /^\d{2}$/.test(parts[0])) {
          jodi = parts[0];
          val = jodi;
